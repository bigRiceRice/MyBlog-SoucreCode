...
1. _Webpack_
    - Webpack 是一个老牌的打包工具与项目构建工具，它的打包工作是自己完成的。

2. _Vite_
    - Vite 是一个新兴的前端项目构建工具，内部集成了大量的第三方包与插件，打包工作由 Rollup 完成。

3. _esbuild_

    - esbuild 是一个将不同的模块化规范转换为 ESM 规范的代码转换工具。

4. _rollup_

    - rollup 是一个基于 ESM 的高效、纯粹的打包工具。
...
首先我们要知道，Vite 使用 `Rollup` （背靠 _ESM_ 规范）为基础来完成打包，它的打包流程大致如下：
...
依赖预构建就是 Vite 会在打包的时候，找到所有依赖文件，对其做一定的封装后调用 esbuild 完成依赖打包（最终 build 使用的是 rollup），然后写入到当前根目录下的 _/node_modules/.vite/deps/_ 中，这一个过程就被称作“**依赖预构建**”。
